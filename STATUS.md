...
- `npm run verify`: aprobado el 2026-09-22; typecheck, 23 pruebas, build web/servidor, prueba HTTP compilada y prueba aislada de restore. `npm run android:sync` y `android/gradlew.bat assembleDebug` aprobados con el JDK Android Studio; para el APK se inyectó `VITE_API_BASE_URL=https://drive.quetzaltech.shop` sin modificar `.env`.
- APK de depuración (no release): `android/app/build/outputs/apk/debug/app-debug.apk`, SHA-256 `5A94891566DD634118B8876C02BDB4D7807A0E4EAFF0DE9BD8AF0C3E4DD33EE4`. Contiene la URL HTTPS de producción, por lo que el aviso de configuración no se activa. Copiado como `quetzaltech-drive.apk` en la raíz, excluido de Git; no se presenta como APK release.
